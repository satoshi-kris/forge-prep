...
class TestAuditCommand(unittest.TestCase):
    ...
    def test_near_dup_threshold_flag_accepted(self):
        code, out, err = run_cli(
            ["audit", str(SAMPLE_CORPUS), "--output", self.output_dir, "--near-dup-threshold", "0.9", "--quiet"]
        )
        self.assertEqual(code, 0)

    def test_shingle_cap_flag_accepted(self):
        code, out, err = run_cli(
            ["audit", str(SAMPLE_CORPUS), "--output", self.output_dir, "--shingle-cap", "50", "--quiet"]
        )
        self.assertEqual(code, 0)

    def test_shingle_cap_zero_means_uncapped(self):
        code, out, err = run_cli(
            ["audit", str(SAMPLE_CORPUS), "--output", self.output_dir, "--shingle-cap", "0", "--quiet"]
        )
        self.assertEqual(code, 0)

    def test_no_near_dup_flag_disables_detection(self):
        code, out, err = run_cli(
            ["audit", str(SAMPLE_CORPUS), "--output", self.output_dir, "--no-near-dup", "--format", "json"]
        )
        self.assertEqual(code, 0)
        data = json.loads(out)
        self.assertEqual(data["audit"]["near_duplicate_count"], 0)
        self.assertEqual(data["audit"]["near_duplicate_clusters"], [])

    def test_near_dup_clusters_appear_in_json(self):
        input_dir = tempfile.mkdtemp()
        try:
            body = (
                "The enterprise segment delivered strong growth this quarter, driven primarily by "
                "expansion within existing accounts rather than new logo acquisition. Net revenue "
                "retention climbed to 118 percent, reflecting successful upsell motions across the base."
            )
            Path(input_dir, "report_v1.txt").write_text(body, encoding="utf-8")
            Path(input_dir, "report_v2.txt").write_text(f"CONFIDENTIAL\n\n{body}\n\nGenerated by Acme v4.2.", encoding="utf-8")
            code, out, err = run_cli(["audit", input_dir, "--output", self.output_dir, "--format", "json"])
            self.assertEqual(code, 0)
            data = json.loads(out)
            self.assertEqual(data["audit"]["near_duplicate_count"], 1)
            self.assertEqual(len(data["audit"]["near_duplicate_clusters"]), 1)
        finally:
            shutil.rmtree(input_dir, ignore_errors=True)
